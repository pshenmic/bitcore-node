...
TimestampService.prototype.onBlock = function(block, callback) {
  console.log('TimestampService.prototype.onBlock ', block.rhash());

  var operations = [];

  var ts = block.ts;
  console.log('var ts = block.ts;', block.ts);
  var hash = block.rhash();

  if (ts <= this._lastBlockTimestamp) {
      console.log('ts <= this._lastBlockTimestamp');
  ...
  this._lastBlockTimestamp = ts;
    ...
    console.log('this._cache.set', hash, ts);
